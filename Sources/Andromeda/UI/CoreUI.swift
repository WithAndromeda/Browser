...
//
    ...
    class ViewModel: NSObject, ObservableObject, WKNavigationDelegate {
        ...
        func addNewTab(withURL url: String = "", title: String = "New Tab") {
            if !url.isEmpty {
                ...
                webConfiguration.websiteDataStore =
                    settingsManager.privacySettings.allowThirdPartyCookies ? .default() : dataStore
        ...
        private func configureWebView(_ webView: WKWebView) {
            ...
            if let matchingRule = settingsManager.privacySettings.siteRules.first(where: {
                $0.matches(url: urlString)
            }) {
                preferences.allowsContentJavaScript =
                    matchingRule.allowJavaScript ?? settingsManager.privacySettings.enableJavaScript
                ...
                preferences.allowsContentJavaScript =
                    settingsManager.privacySettings.enableJavaScript
                let dataStore =
                    settingsManager.privacySettings.allowThirdPartyCookies
                    ? WKWebsiteDataStore.default() : WKWebsiteDataStore.nonPersistent()
                webView.configuration.websiteDataStore = dataStore
            }
    ...
    class SidebarVisibilityManager: ObservableObject {
        ...
        init() {
            NotificationCenter.default.addObserver(
                self, selector: #selector(toggleSidebar),
                ...
                HStack(spacing: 0) {
                    ...
                    VStack(spacing: 0) {
                        if !viewModel.tabs.isEmpty {
                            if let currentURL = viewModel.tabURLs[viewModel.selectedTabIndex],
                                currentURL.isEmpty
                            {
                                HomeView()
                                ...
                                ContentView(
                                    viewModel: viewModel, tabIndex: viewModel.selectedTabIndex)
                            }
                        } else {
                            HomeView()
                                .edgesIgnoringSafeArea(.all)
        ...
        private func tabView(for index: Int) -> some View {
            ...
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill(
                        index == viewModel.selectedTabIndex ? Color.gray.opacity(0.1) : Color.clear)
            )
